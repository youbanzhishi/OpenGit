...
fn collect_files_recursive(repo: &Git2Repo, tree: &Tree, prefix: &str, files: &mut Vec<String>) {
    ...
    for entry in entry_iter {
        if entry.kind() == Some(git2::ObjectType::Blob) {
            ...
            if let Ok(sub_tree) = entry.to_object(repo).and_then(|obj| obj.into_tree().map_err(|_obj| git2::Error::new(git2::ErrorCode::Invalid, "failed to convert object to tree"))) {
